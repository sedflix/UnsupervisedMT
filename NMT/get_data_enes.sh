#!/usr/bin/env bash
#  modified version of get_data_enfr.sh
...
TGT_RAW=$MONO_PATH/all.es
SRC_TOK=$MONO_PATH/all.en.tok
TGT_TOK=$MONO_PATH/all.es.tok
BPE_CODES=$MONO_PATH/bpe_codes
CONCAT_BPE=$MONO_PATH/all.en-es.$CODES
SRC_VOCAB=$MONO_PATH/vocab.en.$CODES
TGT_VOCAB=$MONO_PATH/vocab.es.$CODES
FULL_VOCAB=$MONO_PATH/vocab.en-es.$CODES
SRC_VALID=$PARA_PATH/dev/newstest2013-ref.en
TGT_VALID=$PARA_PATH/dev/newstest2013-ref.es
SRC_TEST=$PARA_PATH/dev/newstest2014-esen-src.en
TGT_TEST=$PARA_PATH/dev/newstest2014-esen-src.es
...
echo "Downloading Spanish files..."
wget -c http://www.statmt.org/wmt14/training-monolingual-news-crawl/news.2007.es.shuffled.gz
wget -c http://www.statmt.org/wmt14/training-monolingual-news-crawl/news.2008.es.shuffled.gz
wget -c http://www.statmt.org/wmt14/training-monolingual-news-crawl/news.2009.es.shuffled.gz
wget -c http://www.statmt.org/wmt14/training-monolingual-news-crawl/news.2010.es.shuffled.gz
# wget -c http://www.statmt.org/wmt14/training-monolingual-news-crawl/news.2011.es.shuffled.gz
# wget -c http://www.statmt.org/wmt14/training-monolingual-news-crawl/news.2012.es.shuffled.gz
# wget -c http://www.statmt.org/wmt14/training-monolingual-news-crawl/news.2013.es.shuffled.gz
# wget -c http://www.statmt.org/wmt15/training-monolingual-news-crawl-v2/news.2014.es.shuffled.v2.gz
# wget -c http://data.statmt.org/wmt17/translation-task/news.2015.es.shuffled.gz
# wget -c http://data.statmt.org/wmt17/translation-task/news.2016.es.shuffled.gz
# wget -c http://data.statmt.org/wmt17/translation-task/news.2017.es.shuffled.gz
...
if ! [[ -f "$SRC_RAW" && -f "$TGT_RAW" ]]; then
  ...
  cat $(ls news*es* | grep -v gz) | head -n $N_MONO > $TGT_RAW
fi
echo "EN monolingual data concatenated in: $SRC_RAW"
echo "ES monolingual data concatenated in: $TGT_RAW"
...
if ! [[ "$(wc -l < $TGT_RAW)" -eq "$N_MONO" ]]; then echo "ERROR: Number of lines doesn't match! Be sure you have $N_MONO sentences in your ES monolingual data."; exit; fi
...
if ! [[ -f "$SRC_TOK" && -f "$TGT_TOK" ]]; then
  ...
  cat $TGT_RAW | $NORM_PUNC -l es | $TOKENIZER -l es -no-escape -threads $N_THREADS > $TGT_TOK
fi
echo "EN monolingual data tokenized in: $SRC_TOK"
echo "ES monolingual data tokenized in: $TGT_TOK"
...
echo "BPE codes applied to ES in: $TGT_TOK.$CODES"
...
echo "ES binarized data in: $TGT_TOK.$CODES.pth"
...
$INPUT_FROM_SGM < $TGT_VALID.sgm | $NORM_PUNC -l es | $REM_NON_PRINT_CHAR | $TOKENIZER -l es -no-escape -threads $N_THREADS > $TGT_VALID
$INPUT_FROM_SGM < $SRC_TEST.sgm | $NORM_PUNC -l en | $REM_NON_PRINT_CHAR | $TOKENIZER -l en -no-escape -threads $N_THREADS > $SRC_TEST
$INPUT_FROM_SGM < $TGT_TEST.sgm | $NORM_PUNC -l es | $REM_NON_PRINT_CHAR | $TOKENIZER -l es -no-escape -threads $N_THREADS > $TGT_TEST
...
echo "    ES: $TGT_TOK.$CODES.pth"
echo "Parallel validation data:"
echo "    EN: $SRC_VALID.$CODES.pth"
echo "    ES: $TGT_VALID.$CODES.pth"
echo "Parallel test data:"
echo "    EN: $SRC_TEST.$CODES.pth"
echo "    ES: $TGT_TEST.$CODES.pth"
echo ""
